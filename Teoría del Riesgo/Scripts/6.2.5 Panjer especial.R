...
PanjerConCeroModificado <- function(r,a,b,f,p0,p1,p0m,g0,todo=F){
  ...
  #(a,b) := Parámetros de familia (a,b,i)
  #f     := vector de probabilidades de Y (ordenadas desde 0)
  #p0    := P[N=0]  (Probabilidad en cero original)
  #p1    .= P[N=1]  (Probabilidad en uno original)
  #p0m   := P[N*=0] (Probabilidad en cero modificada)
  #g0    := P[S=0]
  
  #Le ponemos nombres bonitos a las cosas
  #Creamos un vector auxiliar para las probas de S.
  g<-0:r
  names(g)<-0:r
  #Le ponemos nombres al vector de probas de f.
  ...
  #Calculamos la probabilidad p1m
  p1m = (1-p0m)/(1-p0)*p1
  ...
  for(s in 0:r){
    if(s==0){
      g["0"]=g0
    }else{aux = ifelse(s<=length(f)-1,
                       (p1m-(a+b)*p0m)*f[as.character(s)]/(1-a*f["0"]),0)
    for(j in 1:(min(s,length(f)-1))){
...
#Densidad de los montos de los siniestros (ya con cero):
f <- c(0.25,0.5,0.25)
sum(f)

regresa.una.S<-function(){
  
  #Genera una N modificada
  N=0
  bandera<-sample(x = 0:1,size = 1,replace = T,prob = c(pi/4,1-pi/4))
