...
GEN_ID_CAE <- function(ID_EDO_DIST, ARE){
...
id_unico_casilla <- function(BD){
  paste0("A", formatC(BD$ID_ESTADO, width = 2, flag = 0),
         formatC(BD$ID_DISTRITO, width = 2, flag = 0), 
...
CREA_BASE_X_DISTRITO_X_PARTIDO <- function(BD){
...
CALCULA_MR_VTE_RELOADED <- function(my_contendientes, INFO_COAL, BD2){
  VTEd <- colSums(BD2)
  # ¿ES EFICIENTE?
  BD2[INFO_COAL$hay_vm,1:3] <- rowSums(BD2[INFO_COAL$hay_vm,1:3])*INFO_COAL$COAL_VM2
  BD2[INFO_COAL$hay_jh,c(4,5,7)] <- rowSums(BD2[INFO_COAL$hay_jh,c(4,5,7)])*INFO_COAL$COAL_JH2
  #
  MR <- apply(BD2[,-c(12,13)], MARGIN = 1,FUN = function(x){my_contendientes[which.max(x)]})
...
CALCULA_NP_VVE <- function(my_partidos, res1){
  ...
  while(kex > 0) {
    ...
    Gj <- 1*(Gj | (Ej > 0))
    # Paso 4:
    RP <- pmax(zeros, RP - Ej) # SOLO X SEGURIDAD PARA NO ACABAR CON NEGATIVOS
    # Paso 5:
    M <- sum(Ej)
