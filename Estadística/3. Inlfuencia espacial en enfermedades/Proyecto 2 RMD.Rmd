...
Comenzamos mostrando un mapa del estado de Carolina del Norte (Figura \ref{fig:f1}). Este gráfico lo estaremos mostrando en repetidas ocasiones buscando explicar con estilo mapa de calor interacciones con diferentes resultados obtenidos y postulados en este proyecto. En las tablas al final del documento (Anexo A y B) podremos ver los nombres de los condados con base en el número expuesto.

\begin{figure}

```{r f1, echo=FALSE}
# Graficamos Carolina del Norte con el ID alfabético por condado
plot(st_geometry(nc), axes=FALSE, main = "Mapa de condados de Carolina del Norte")
text(st_coordinates(st_centroid(st_geometry(nc), of_largest_polygon=TRUE)), label=nc$CRESS_ID, cex=0.5)
```
\vspace{-3cm}
\caption{Estado de Carolina del Norte con ID.\label{fig:f1}}
...
De las Figuras \ref{fig:f8} y \ref{fig:f9} vemos que el modelo para el periodo del 74 NO parece explicar el efecto espacial por lo que quizás necesitaríamos más variables para hacerlo, mientras que para el periodo del 79 logramos hacer que el modelo absorba el efecto espacial y esto se ve con los residuales tomando un comportamiento que parece "más" aleatorio. Esto se puede verificar con la prueba de Moran realizada en la Tabla 13.
...
# OJO, Las simulaciones sí van ordenadas.
