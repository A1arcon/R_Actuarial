...
# Quiero encontrar la x tal que f(x) = y 
f <- function(x,y) { x^2 - y } # En este caso f(x) = x^2
...
#Opción 2 YA REGRESÓ :) ¡pero resuelve otras cosas..!
library(pracma)
?fsolve 
fsolve(f,y=3^2,x0 = 100)$x # ¡Sirve para otras cosas!
...
Prima.Utilidad.Cero <- function(v,u,S,lower=0,upper=10^100,tol=10^-100){
  ...
  f<-function(p){
    ...
    vupS <- sapply(X = u+p-S,FUN = v) #v(u+p-S)
    v(u)-mean(vupS)
...
#Función generadora de momentos de S
#E[exp(tS)]=\sum_{\forall s} P(S=s)*exp(s*t) = Σ_{s} P(S=s)*exp(s*t)
Ms<-function(t){
  sum(exp(0:6*t)*gx)
}
Ms(0)
