...
MASS::truehist(datos,asp=1)
abline(v=(n-1)/lambda,col="red",lwd=2) 
...
abline(v=bawr,col="darkblue",lwd=2)
bawr[2]-bawr[1]
...
qqplot(datos, teoricos,
       main = latex2exp::TeX("Q-Q plot $Gamma(n,\\lambda)$")
...
##Teórica
curve(dgamma(x,n,lambda),from=0,to=25,add=TRUE,lwd=2)
...
# ~~~~~~ Esta función sirve para hacer el análisis con una app shiny ~~~~~~ #
library(rriskDistributions)
gof <- fit.cont(datos[1:1000])
gof
...
ks.test(datos,Y)
ks.test(datos[1:(k/2)],datos[(k/2+1):k])
...
ks.test(datos, "pgamma",shape = a^2/(b-a^2),rate = a/(b-a^2))
...
goftest::ad.test(datos, "pgamma",shape = a^2/(b-a^2),rate = a/(b-a^2))
