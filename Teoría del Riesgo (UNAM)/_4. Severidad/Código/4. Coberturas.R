...
MASS::truehist(YL,probability = T,col="red")
...
X<-runif(100000,min,max)
# points(X,f(X),pch=4)
# points(X[D<X & X<U],f(X)[D<X & X<U],pch=4,col="purple")
Y<-pmax(pmin(X,U)-D,0)
...
# Caso particular (deducible mínimo) -----------------------------------------
...
plot(ecdf(Y))
# El primer salto
abline(h=fY(0),col="red")
# El otro salto
abline(h=1-fY(alpha*(U-D)),col="blue")
